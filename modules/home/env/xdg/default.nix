{
  ...
  inherit (lib) types toList mapAttrs' nameValuePair;
  inherit (lib.strings) toUpper;
  inherit (lib.options) mkOpt';
  cfg = config.sprout.env.xdg;
in {
  options.sprout.env.xdg = with types; let
  ...
  config = {
    ...
    home.sessionVariables = mapAttrs' (n: v:
      nameValuePair ("XDG_" + toUpper n + "_HOME") v)
    cfg.homedirs;
